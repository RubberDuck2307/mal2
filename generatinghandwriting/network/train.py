...
if (run_name != "output/test") and os.path.exists(save_path):
    raise ValueError(f"Directory {save_path} already exists!")
...
for epoch in range(num_epochs):
    ...
    with open(f"{save_path}/stats", "a") as f:
        ...
        f.write('[%d/%d][%d/%d]\tLoss_D: %.4f\tLoss_G: %.4f\tD(x): %.4f\tD(G(z)): %.4f / %.4f \t time:[%d] \n'
          % (epoch, num_epochs, i, len(dataloader),
             errD.item(), errG.item(), D_x, D_G_z1, D_G_z2, time.seconds))
